...
// captures nothing — verified live, and re-verified 2026-09-06: a main-world
// fetch AND XHR hook see zero requests while the grid grows 10 -> 33 tiles), so
// unlike Instagram we read the grid tiles straight from the DOM: reel id,
// thumbnail, and the localized VIEW COUNT text.
//
// One trap when probing this by hand: the grid only paginates while the tab is
// genuinely VISIBLE. With the window occluded (document.visibilityState
// "hidden") it stops at the 10 server-rendered reels and looks capped.
...
// >>> inline:end
  ...
  // A numeric profile's reels grid is /profile.php?id=…&sk=owner_reels — its path
  // is /profile.php, so the old path-and-"reels_tab" test said "not the reels tab"
  // and collect() returned an empty list on the exact URL the panel was pointed at.
  // isReelsGridUrl (inlined above) knows all three shapes.
  const isReelsTab = () => isReelsGridUrl(location.href);
