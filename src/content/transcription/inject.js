...
// >>> inline:end
  // <<< inline:src/lib/shared/fbReelThumb.js
// GENERATED by scripts/gen-inline.mjs — do not edit here.
// Edit src/lib/shared/fbReelThumb.js and run `npm run gen:inline`.
...
// >>> inline:end
  ...
  // ---- profile reels grid: one cover-download button per tile ----
  // The tile's <img> IS the reel's cover, but Facebook serves it cropped to
  // 540x960; fullResThumb asks the CDN for the native 1080x1920 frame (see
  // src/lib/shared/fbReelThumb.js for why dropping `ctp` is the whole trick).
  const TILE_TIP = "Baixar miniatura (1080×1920)";
  // The reels grid renders no <h1> on profile.php — same reason reels-capture.js
  // has to resolve the owner from the profile links — so fall back to the tab
  // title, which names the profile on every shape of this surface.
  function reelGridOwner() {
  ...
  // Unlike the video/photo rails this one reads the background's own reply instead
  // of the shared `pending` queue: a wall of tiles invites several clicks in
  // flight at once, and that queue resolves FIFO, so the tick would land on
  // whichever button happened to be first in line rather than the one that failed.
  async function btnReelThumbJob(img, reelId, btn) {
    if (btn.classList.contains("busy")) return;
    const url = fullResThumb(img.currentSrc || img.src);
    if (!url) return;
    setBtnState(btn, "busy");
    const author = sanitFb(reelGridOwner()) || "page";
    let ok = false;
    try {
      // Flat name, no per-profile sub-folder: the file name already says whose
      // reel this is, and a directory per profile buried single covers one click
      // deeper than the panel's own downloads. Same shape the panel's Reels Sort
      // produces (lib/fbReels.js filenameFor), so all three buttons agree.
      const res = await chrome.runtime.sendMessage({
        type: "FBW_DL_MEDIA", platform: "facebook", kind: "image", folder: "thumb",
        url, filename: `fb-${author}-${reelId}.jpg`,
      });
      ok = !!(res && res.ok);
    } catch { ok = false; }
    if (!btn.isConnected) return;
    setBtnState(btn, ok ? "ok" : "err");
    btn.title = btn.dataset.tip = ok ? "Miniatura baixada" : "Falha ao baixar a miniatura";
    setTimeout(() => {
      if (!btn.isConnected) return;
      setBtnState(btn, "idle");
      btn.title = btn.dataset.tip = TILE_TIP;
    }, 2500);
  }
  ...
  // Grid tiles are 160x280 and the bottom-left corner carries FB's view count, so
  // this rail sits top-right and out of the way of the tile's own chrome.
  function buildReelTileRail(img, reelId) {
    const wrap = document.createElement("div");
    wrap.className = "fbw-acts tile";
    wrap.appendChild(mkBtn("download", TILE_TIP, (b) => btnReelThumbJob(img, reelId, b)));
    return wrap;
  }
  ...
  function surfaceAllowsMediaButtons() {
    ...
    return (
      ...
      // The profile reels grid: every tile gets its own cover-download button.
      isReelsGridUrl(location.href)
    );
  }
  ...
  function decorateMedia() {
    ...
    // Profile reels grid: every tile is an <a href="/reel/<id>"> around its cover.
    ...
    if (isReelsGridUrl(location.href)) {
      for (const a of document.querySelectorAll('a[href*="/reel/"]')) {
        ...
        if (r.bottom < -vh || r.top > vh * 2) continue;
        const reelId = reelIdFromHref(a.getAttribute("href"));
        const img = a.querySelector("img");
        if (!reelId || !img) continue;
        if (!a.querySelector(":scope > .fbw-acts")) targets.push([a, "reel-thumb", img, reelId]);
      }
    }
    ...
    for (const [anchor, kind, el, reelId] of targets) {
      ...
      anchor.appendChild(
        kind === "video"
          ? buildVideoRail(el, onReel)
          : kind === "reel-thumb"
            ? buildReelTileRail(el, reelId)
            : buildImageRail(el),
      );
    }
  ...
  let thumbBtn = null;
  function onReelsTab() {
    // Used to test `pathname + search` against /reels(\/|$)/, which missed the
    // shape a numeric profile actually gets — /profile.php?id=…&sk=owner_reels —
    // so this button never appeared there. isReelsGridUrl knows all three shapes.
    if (!isReelsGridUrl(location.href)) return false;
  ...
  function ensureThumbBtn() {
    ...
    thumbBtn.addEventListener("click", async (e) => {
      ...
      const harvest = () => {
        for (const a of document.querySelectorAll('a[href*="/reel/"]')) {
          const id = reelIdFromHref(a.getAttribute("href"));
          const img = a.querySelector("img");
          // Store the NATIVE 1080x1920 URL, not the 540x960 the grid paints.
          if (id && img && img.src) seen.set(id, fullResThumb(img.currentSrc || img.src));
        }
      };
      ...
      for (const [id, url] of seen) {
        chrome.runtime.sendMessage({
          ...
          // extension makes: the name already carries the profile, so the folder
