...
// <<< inline:src/lib/shared/ttPlayerHost.js
// GENERATED by scripts/gen-inline.mjs — do not edit here.
// Edit src/lib/shared/ttPlayerHost.js and run `npm run gen:inline`.
 ...
 */
function isTtPlayerSurface(pathname, centered) {
...
// >>> inline:end
  ...
  async function ttTranscribe(rec, language) {
    ...
    chrome.runtime.sendMessage({
      ...
      // The post's own metadata, straight off the record the overlay is already
      // drawing from — no request. Same three fields the Instagram rail forwards.
  ...
  function renderPlayerRail(centered) {
    ...
    // of the rail over a tile that already carries its own.
    //
    // The test is the tile's own permalink, not `a[data-sw-id]` as it once was:
    // that attribute is OUR annotation, so it is only present on tiles we already
    // built a rail for. A tile whose record has not been captured yet carries no
    // stamp, and used to slip through as a player.
    if (isTtTileMedia(centered)) centered = null;
  ...
  function maintainOverlay() {
    ...
    // A tile's autoplaying preview is not a player — see ttPlayerHost.js. Deciding
    // this on "some video is on screen" put the whole rail on every grid, pointing
    // at whichever tile drifted closest to the middle.
    const onVideo = isTtPlayerSurface(location.pathname, centered);
